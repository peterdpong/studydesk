...
import { findCalendarMatch } from '../../lib/db-actions/ClassActions'
...
export default function MainCalendar(props: {
  ...
  const onChange = (nextValue: Date) => {
    setValue(nextValue)
    const dateString = nextValue.toISOString().substring(0, 10)

    const dayString = nextValue.toString().substring(0, 3)

    setDate(dateString.substring(5, 10).replace('-', '/'))
    setDay(dayString)

    const { assignmentArray, taskArray, classArray } = findCalendarMatch(
      props.tasks,
      props.classes,
      dateString,
      dayString
  ...
  return (
    <Box mt={5}>
      <Heading mb={2} textAlign={{ base: 'center', md: 'left' }}>
        Calendar
      </Heading>
